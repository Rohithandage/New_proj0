...
import { getUserCurrencyInfo, formatPrice, convertPrice, getCachedCurrencyInfo } from "@/lib/currency-utils";
import apiConfig from "@/config/api";
import { preloadCriticalImage, preloadImages } from "@/lib/image-preloader";

// Cache admin currency settings to avoid repeated API calls
const ADMIN_CURRENCY_CACHE_KEY = 'admin_currency_settings';
const ADMIN_CURRENCY_CACHE_TIMEOUT = 60 * 60 * 1000; // 1 hour

const getCachedAdminCurrencySettings = () => {
  try {
    const cached = localStorage.getItem(ADMIN_CURRENCY_CACHE_KEY);
    if (!cached) return null;
    const { data, timestamp } = JSON.parse(cached);
    if (Date.now() - timestamp < ADMIN_CURRENCY_CACHE_TIMEOUT) {
      return data;
    }
    localStorage.removeItem(ADMIN_CURRENCY_CACHE_KEY);
    return null;
  } catch {
    return null;
  }
...
const cacheAdminCurrencySettings = (data) => {
  try {
    localStorage.setItem(ADMIN_CURRENCY_CACHE_KEY, JSON.stringify({
      data,
      timestamp: Date.now()
    }));
  } catch {}
...
// Normalize price - handle MongoDB extended JSON format or plain numbers
...
const ProductCard = memo(({ product, isDetailPage = false }) => {
  ...
  // Initialize with cached currency info immediately for instant display
  ...
  useEffect(() => {
    if (allImages.length > 0 && !imagePreloadedRef.current) {
      imagePreloadedRef.current = true;
      
      // Preload first image with highest priority (critical)
      preloadCriticalImage(allImages[0]);
      
      // Preload next 2 images immediately (not in background)
      if (allImages.length > 1) {
        preloadImages([allImages[1]], 'high', 2);
      }
      if (allImages.length > 2) {
        preloadImages([allImages[2]], 'high', 2);
      }
      
      // Preload remaining images in background
      if (allImages.length > 3) {
        preloadImages(allImages.slice(3), 'low', 4);
      }
    }
  }, [allImages]);
  
  // Load currency info in background (non-blocking) - use requestIdleCallback for better performance
  useEffect(() => {
    ...
    const loadCurrencyInfo = async () => {
      try {
        // Check cache first for admin settings
        let adminCurrencySettings = getCachedAdminCurrencySettings();
        
        // Fetch admin currency settings if not cached (non-blocking)
        if (!adminCurrencySettings) {
          // Use fetch with low priority
          fetch(`${apiConfig.BASE_URL}/api/settings/currency`, {
            priority: 'low' // Browser hint for low priority
          })
            .then(response => response.json())
            .then(data => {
              if (data.success && data.data) {
                cacheAdminCurrencySettings(data.data);
              }
            })
            .catch(() => {
              // Silently fail
        ...
        // Get user currency info (uses cache internally) - non-blocking
        getUserCurrencyInfo(adminCurrencySettings)
          .then(info => {
            setCurrencyInfo(info);
          })
          .catch(() => {
            // Silently fail - keep default/cached currency
          });
      } catch (error) {
        // Silently fail - keep default/cached currency
      }
    ...
    if (typeof requestIdleCallback !== 'undefined') {
      requestIdleCallback(loadCurrencyInfo, { timeout: 2000 });
    } else {
      setTimeout(loadCurrencyInfo, 0);
    }
  }, []);
  ...
  const handleExternalLink = useCallback((url, website, e) => {
    e.preventDefault();
    
    // Open the affiliate link immediately (non-blocking)
    window.open(url, '_blank', 'noopener,noreferrer');
    
    // Track affiliate click in background (non-blocking, fire and forget)
    // Use cached currency info for country if available
    const trackingData = {
      productId: product._id,
      productName: product.name,
      website: website,
      affiliateUrl: url,
      country: currencyInfo?.countryName || 'Unknown',
      countryCode: currencyInfo?.countryCode || null,
      userAgent: navigator.userAgent
    };
    
    // Send tracking request asynchronously (doesn't block)
  ...
  }, [product._id, product.name, currencyInfo]);
  ...
  // Prefetch product details page on hover for faster navigation
  useEffect(() => {
            ...
            <>
              <div className="relative w-full h-full flex items-center justify-center">
                {allImages.map((image, index) => {
                  ...
                  const isNext = index === (currentImageIndex + 1) % allImages.length;
                  ...
                  return (
                    <img
                      ...
                      loading={index === 0 ? "eager" : isNext ? "eager" : "lazy"}
                      fetchPriority={index === 0 ? "high" : isCurrent ? "high" : "low"}
                      className={`absolute ${
                        isDetailPage 
  ...
  // Custom comparison function for memo - optimized for performance
  if (prevProps.product?._id !== nextProps.product?._id) return false;
  if (prevProps.isDetailPage !== nextProps.isDetailPage) return false;
  
  // Deep compare prices array only if length changed
  if (prevProps.product?.prices?.length !== nextProps.product?.prices?.length) return false;
  
  // Check if prices actually changed (compare first and last price values)
  const prevPrices = prevProps.product?.prices || [];
  const nextPrices = nextProps.product?.prices || [];
  if (prevPrices.length > 0 && nextPrices.length > 0) {
    const prevFirst = prevPrices[0]?.price;
    const nextFirst = nextPrices[0]?.price;
    const prevLast = prevPrices[prevPrices.length - 1]?.price;
    const nextLast = nextPrices[nextPrices.length - 1]?.price;
    if (prevFirst !== nextFirst || prevLast !== nextLast) return false;
  }
  
  // Check if images changed
  const prevImages = prevProps.product?.images || (prevProps.product?.image ? [prevProps.product.image] : []);
  const nextImages = nextProps.product?.images || (nextProps.product?.image ? [nextProps.product.image] : []);
